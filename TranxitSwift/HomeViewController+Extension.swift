...
extension HomeViewController {
    ...
    func showInvoiceView(with request : Request) {
        ...
        if self.invoiceView == nil, let invoice = Bundle.main.loadNibNamed(XIB.Names.InvoiceView, owner: self, options: [:])?.first as? InvoiceView {
            ...
            self.invoiceView?.onClickPaynow = { tipsAmount in
                ...
                if self.invoiceView?.paymentType == .BRAINTREE ||  self.invoiceView?.paymentType == .CARD ||  self.invoiceView?.paymentType == .PAYTM || self.invoiceView?.paymentType == .PAYUMONEY{
    ...
    func showProviderInCurrentLocation(with data : [Provider],serviceTypeID:Int) {
        ...
        for locationData in data where locationData.longitude != nil && locationData.latitude != nil {
            ...
            //Icones serviços
            let serviceMototaxiID = 2;
            let serviceMotoboyID = 2;
            ...
            if serviceTypeID == 0 {
                ...
                if(locationData.service?.service_type_id == serviceMototaxiID || locationData.service?.service_type_id == serviceMotoboyID){
                    marker.icon = #imageLiteral(resourceName: "map-moto-icon-purple").resizeImage(newWidth: 20)
                }else{
                    marker.icon = #imageLiteral(resourceName: "map-vehicle-icon-purple").resizeImage(newWidth: 20)
                }
    ...
    func clearMapview(clearMarkers: Bool = true) {
        self.mapViewHelper?.mapView?.clear()
