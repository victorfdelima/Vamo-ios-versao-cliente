...
//            self.buttonChangePayment.isHidden = true
    ...
    @IBAction private func buttonPaynowAction() {
        if buttonPayNow.titleLabel?.text == Constants.string.Done{
            if request?.paid == 0 {
                self.makeToast(Constants.string.confirmPayment.localize(), duration: 2.0, position: .center)
                return
            }
            ...
            // Status e bloqueio no processamento do pagamento
            buttonPayNow.setTitle("Processando...", for: .normal)
            buttonPayNow.isEnabled = false
            buttonPayNow.alpha = 0.45
        }
    }
