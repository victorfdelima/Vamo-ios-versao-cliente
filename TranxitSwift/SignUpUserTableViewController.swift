...
class SignUpUserTableViewController: UITableViewController {
    ...
    @IBOutlet var cpfText: HoshiTextField!
...
extension SignUpUserTableViewController {
    ...
    @IBAction func nextBtnTapped(sender : UITapGestureRecognizer){
        ...
        guard let cpf = cpfText.text, !cpf.isEmpty else {
            self.showToast(string: "Por favor, informe seu CPF")
            return
        }
        ...
        userInfo =  MakeJson.signUp(loginBy: .manual, email: email, password: password, socialId: nil, firstName: firstName, lastName: lastName, cpf: cpf, mobile: mobile, referral_code: isReferalEnable == 0 ? "" : self.textFieldReferCode.text!, country_code: self.countryText.text)
