...
class ProfileViewController: UITableViewController {
    ...
    @IBOutlet private weak var textFieldCpf : HoshiTextField!
    ...
    @IBOutlet weak var labelAvisoCadastro: UILabel!
    ...
    var shouldCompleteRegistration: Bool = false
...
extension ProfileViewController {
    ...
    private func setProfile(){
        ...
        self.textFieldCpf.text = User.main.cpf
        ...
        if(shouldCompleteRegistration){
            labelAvisoCadastro.isHidden = false
        }
    }
    ...
    private func setDesign() {
        ...
        [textFieldFirst, textFieldLast, textFieldCpf, textFieldEmail, textFieldPhone].forEach({
        ...
        Common.setFont(to: textFieldCpf)
    ...
    @IBAction private func buttonSaveAction(){
        ...
        guard let firstName = self.textFieldFirst.text, firstName.count>0 else {
            UIApplication.shared.keyWindow?.make(toast: ErrorMessage.list.enterFirstName.localize())
            return
        }
        
        guard let lastName = self.textFieldLast.text, lastName.count>0 else {
            UIApplication.shared.keyWindow?.make(toast: ErrorMessage.list.enterLastName.localize())
            return
        }
        
        guard let cpf = self.textFieldCpf.text, cpf.count>0 else {
            UIApplication.shared.keyWindow?.make(toast: "Por favor, informe um CPF")
            return
        }
        
        guard let mobile = self.textFieldPhone.text, mobile.count>0 else {
            UIApplication.shared.keyWindow?.make(toast: ErrorMessage.list.enterMobileNumber.localize())
            return
        }
        
        guard self.imageViewProfile.image != #imageLiteral(resourceName: "userPlaceholder") else {
            UIApplication.shared.keyWindow?.make(toast: "Por favor, inserir imagem de perfil")
            return
        }
        ...
        //            UIApplication.shared.keyWindow?.make(toast: ErrorMessage.list.enterEmail.localize())
        //            return
        //        }
        
        //        guard Common.isValid(email: email) else {
        //            UIApplication.shared.keyWindow?.make(toast: ErrorMessage.list.enterValidEmail.localize())
        //            return
        //        }
    ...
    private func localize() {
        ...
        self.textFieldCpf.placeholder = "CPF"
    ...
    @IBAction private func editBtnAction(sender: UIButton) {
        ...
        self.accountKit = AKFAccountKit(responseType: .accessToken)
        let akPhone = AKFPhoneNumber(countryCode: "br", phoneNumber: "")
        let accountKitVC = accountKit?.viewControllerForPhoneLogin(with: akPhone, state: UUID().uuidString)
        accountKitVC!.enableSendToFacebook = true
        self.prepareLogin(viewcontroller: accountKitVC!)
        self.present(accountKitVC!, animated: true, completion: nil)
