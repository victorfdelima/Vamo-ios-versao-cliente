    ...
    class HomeViewController: UIViewController {
        ...
        lazy var markerProviderLocation : GMSMarker = {  // Provider Location Marker
            //Icones serviços
            let serviceMototaxID = 98;
            let serviceMotoboyID = 97;
            let marker = GMSMarker()
            let imageView = UIImageView(frame: CGRect(origin: .zero, size: CGSize(width: 30, height: 30)))
            imageView.contentMode =  .scaleAspectFit
            
            if(selectedService?.id == serviceMototaxID || selectedService?.id == serviceMotoboyID ){
                imageView.image = #imageLiteral(resourceName: "map-moto-icon-purple")
            }else{
                imageView.image = #imageLiteral(resourceName: "map-vehicle-icon-purple")
            }
    ...
    extension HomeViewController {
        ...
        private func initialLoads() {
            ...
            //Verifica se o usuário tem CPF
            if(User.main.cpf == nil){
                UIApplication.shared.keyWindow?.make(toast: "Complete seu cadastro!")
                
                //                if let vc = self.storyboard?.instantiateViewController(withIdentifier: Storyboard.Ids.ProfileViewController) {
        ...
        func showUserProfile(){
            let homeVC = Router.user.instantiateViewController(withIdentifier: Storyboard.Ids.ProfileViewController) as! ProfileViewController
            homeVC.shouldCompleteRegistration = true
            (self.drawerController?.getViewController(for: .none) as? UINavigationController)?.pushViewController(homeVC, animated: true)
        }
        
        // Redirect user if cpf or picture is invalid
        func checkUserProfile(){
            
            // Check profile picture
            guard let url = User.main.picture, !url.isEmpty else{ self.showUserProfile(); return}
            // Check cpf
            guard let cpf = User.main.cpf else{ self.showUserProfile(); return}
            // Check if cpf is empty
            guard !cpf.replacingOccurrences(of: " ", with: "").isEmpty else{ self.showUserProfile(); return}
            ...
            if currentLocation.value?.latitude != nil || currentLocation.value?.longitude != nil {
                self.currentLocation.bind(listener: { (locationCoordinate) in
                    ...
                    if locationCoordinate != nil {
                        ...
                        //TODO ALLAN - Consulta motoristas
                        let json = [Constants.string.latitude : locationCoordinate?.latitude ?? defaultMapLocation.latitude, Constants.string.longitude : locationCoordinate?.longitude ?? defaultMapLocation.longitude] as [String : Any]
        ...
        @IBAction private func sideMenuAction(){
            ...
            if self.isOnBooking { // If User is on Ride Selection remove all view and make it to default
                self.clearAllView(clearMarkers: false)
        ...
        func clearAllView(clearMarkers: Bool = true) {
            self.removeLoaderView()
            self.removeUnnecessaryView(with: .cancelled)
            self.clearMapview(clearMarkers: clearMarkers)
        ...
        func mapView(_ mapView: GMSMapView, didChange position: GMSCameraPosition) {
            ...
            if isUserInteractingWithMap {
                ...
                if self.selectedLocationView == self.viewSourceLocation, self.sourceLocationDetail != nil {
                    ...
                    self.sourceMarker.position = position.target
                    ...
                    self.destinationMarker.position = position.target
